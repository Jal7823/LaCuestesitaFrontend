...
import { setToken } from '../../reducer/userToken';
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(setToken({ token: response.data.access })); // Almacenar el token en Redux
      // Aquí puedes redirigir a otro componente o actualizar la vista
      // Por ejemplo: window.location.href = '/admin'; // Redirigir a la sección de admin
    } catch (err) {
      setError('Login failed: ' + err.response.data.detail); // Manejo de errores
    }
  };

  return (
    <div className="bg-gray-50 dark:bg-custom-primary-black font-[sans-serif]">
      <div className="min-h-screen flex flex-col items-center justify-center py-6 px-4">
        <div className="max-w-md w-full">
          <a href="javascript:void(0)">
            <img src="logo.jpeg" alt="logo" className="w-40 mb-8 mx-auto block" />
          </a>
          <div className="p-8 rounded-2xl bg-white shadow">
            ...
            <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
              ...
              {error && <p className="text-red-600 text-sm text-center mt-4">{error}</p>} {/* Mensaje de error */}
